...
the free parameers are {Tkin_b, Tkin_t, nH2, NCS, M, x0, x1, x2}.
As the slab models are relatively simple we do not consider flux calibration
uncertainties, correlated noise or different Mach values for each line. We do,
however, consider a gradient in the temperature.
...
class fitdict:
    ...
    def _lnprior(self, theta):
        ...
        """
        temp_b, temp_t, dens, sigma, mach, x0s = self._parse(theta)
        if not temp_b < temp_t:
            return -np.inf
        if not self.grid.in_grid(temp_b, 'temp'):
            return -np.inf
        if not self.grid.in_grid(temp_t, 'temp'):
            return -np.inf
    ...
    def _parse(self, theta):
        ...
        return theta[0], theta[1], theta[2], theta[3], theta[4], x0s
    ...
    def _lnlike(self, theta):
        ...
        spectra = [[self._spectrum(j, temp, dens, sigma, v, x0, mach)
                    for temp in [temp_b, temp_t]]
                   for j, v, x0 in toiter]
        return self._lnx2(np.average(spectra, axis=1))
    ...
    def emcee(self, **kwargs):
        ...
        self.params = ['temp_b', 'temp_t', 'dens', 'sigma', 'mach']
        ...
        # each parameter and start from there. We also have to rearrange the
        # first two columns to make sure that temp_b < temp_t.
