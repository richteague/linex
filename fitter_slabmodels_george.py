...
class fitdict:
    ...
    def _lnprior(self, theta):
        ...
        """
        temp, dens, sigma, mach, x0s, vcorr = self._parse(theta)
    ...
    def _parse(self, theta):
        ...
        x0s = [theta[j] for j in -(np.arange(len(self.trans)) + 2)[::-1]]
        return theta[0], theta[1], theta[2], theta[3], x0s, theta[-1]
    ...
    def _lnlike(self, theta):
        ...
        lnx2 = 0.0
        noises = [george.GP(dy**2 * ExpSq(10**vcorr)) for dy in self.rms]
